...
# Select only helpful variables from metadata:
metadata %>% dplyr::select('series_id')
...
```{r}
ggplot(corrected.pca.df, aes(x = PC1, y = PC2, shape = as.factor(metadata$refinebio_platform), 
       color = metadata$series_id)) +
  geom_point() + 
  theme(legend.position = "none") +
