...
*Purpose*: This notebook illustrates how to assess and correct for batch effects
with `limma`. 

It is good practice to assess potential batch effects in all datasets.
For most datasets, the [refine.bio normalization pipeline](http://docs.refine.bio/en/latest/main_text.html)
completed prior to your download, may have sufficiently minimized batch effects.

Traditionally, batch correction is thought of in terms of technical batches, 
or in other words, what samples were processed when and in what groups. 
However, most publicly available data does not include this information. 
Another application of batch correction is to use it to smooth over differences 
among datasets aggregated together.

For datasets with similar set ups, we can aggregate data using the
...
comparable, we still will expect data from different platforms to show 
differences. 
This is something we can try to use batch correction to deal with. 
...
#### 3) Perform pre and post evaluations of any batch correction  
