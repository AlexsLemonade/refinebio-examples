...
First, we need to calculate the gene means, but we'll need to move our non-numeric variables (the gene ID columns) out of the way for that calculation. 
...
This is probably mostly because they share many of the same genes, but to a certain smaller extent because they are biologically related. 

Taking this example, how many genes are in common for `HALLMARK_INTERFERON_ALPHA_RESPONSE` and `HALLMARK_INTERFERON_GAMMA_RESPONSE`? 
...
The pathway gene sets are all very overlapping!
