...
Rather than summarize the values for multiple Ensembl gene IDs that map to a single Entrez ID (e.g., take the mean or median), we will use the values associated with the Ensembl gene ID that seems to be more highly expressed.
This strategy should maintain a bit more clarity of what these values are representing in terms of the original reads. 

Our example doesn't contain too many duplicates so ultimately we only are losing 68 rows of data.
But if you find yourself using a dataset that has large proportion of duplicates you may want to consider doing some interrogation into the data and Ensembl gene IDs you are dropping by performing correlations between Ensembl ID's data and writing a file of the dropped genes so you have it for record. 
