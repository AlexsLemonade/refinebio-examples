...
You can use these scores for other downstream analyses like differential expressioon. 
...
For RNA-seq processing in refine.bio, reads were mapped to these Ensembl IDs, so as opposed to combining two (or more) Ensembl IDs data in a summary stat and getting a value that's not clearly representing any particular mapping, it might be a more representative of the original reads to just choose one Ensembl ID's data over the other(s). 

To do this, we first need to calculate the gene means, but we'll need to move our non-numeric variables (the gene ID columns) out of the way for that calculation. 
...
If we do an `dplyr::arrange()` by `gene_means` first, then this means the first row encountered by `dplyr::distinct()` will be the one with the higher gene mean and by default the first row of any duplicates is kept. 
...
Our main information about these samples is squished into one column, `refinebio_title`. 
...
This information is not very clear as is. 
...
We can also see the conceptually similar pathways seem to cluster together (e.g. `HALLMARK_INTERFERON_ALPHA_RESPONSE` and `HALLMARK_INTERFERON_GAMMA_RESPONSE`). 
Often pathways may look similar because their gene sets are not distinct from each other and can have a lot of overlap. 
