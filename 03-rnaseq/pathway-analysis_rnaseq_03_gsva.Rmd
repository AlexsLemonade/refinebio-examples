...
In other words, we will narrow down one Ensembl ID's data for a particular Entrez gene identifier instances for each sample or column in this case.
...
First we need to calcuate the gene means, but we'll need to move our non-numeric variables (the gene ID columns) out of the way. 
...
Let's check on one of the duplicate Entrez IDs and see if we only get one row back. 
...
Here we are using `kcdf = "Gaussian"` because we are using continous, normalized and transformed data. 
