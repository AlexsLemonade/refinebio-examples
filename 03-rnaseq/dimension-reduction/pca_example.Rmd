...
For this example analysis, we will use this [<<BRIEF DATASET NAME>>](https://www.refine.bio/experiments/<<REFINEBIO DATSET LINK>> )..
...
In this analysis, we will be using the R package [`DESeq2`](http://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html) [@DESeq2] for normalization and plotting of PCA values.
...
In this code chunk, the variables "refinebio_treatment" and "refinebio_disease" were given to the `plotPCA()` function due to the hypothesis of the experiment stating "that differences in tumor transcriptional programs may reflect differential responses to androgen deprivation therapy (ADT) and subsequent metastasis".
...
# pcaData <- plotPCA(dds_norm, intgroup = c("refinebio_treatment", "refinebio_disease"), returnData = TRUE)
#
# ggplot(pcaData, aes(
#   x = PC1,
#   y = PC2,
#   color = factor(refinebio_treatment),
#   shape = factor(refinebio_disease)
# )) +
#   geom_point(size = 3)
```
