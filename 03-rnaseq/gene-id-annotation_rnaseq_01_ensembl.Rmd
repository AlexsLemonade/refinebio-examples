...
mapped_df <- mapped_list %>%
  ...
  tidyr::unnest() # enframe makes a `list` column, so here we are converting that to simpler format
```
...
Let's take a look at our new collapsed `entrez_ids` column in the `final_mapped_df` object.

```{r}
final_mapped_df %>%
  # Filter `final_mapped_df` to include only the rows where `entrez_ids` values include the ";" character -- we know that these are the rows with multiple mapped values
...
Let's re-run `mapIds()`, this time using `multiVals = "first"` and add this information to our `final_mapped_df` object.
...
final_mapped_df <- data.frame(
  ...
  dplyr::inner_join(final_mapped_df, by = c("Ensembl" = "Gene"))
```

Our `final_mapped_df` object now has a column named `first_mapped_entrez_id` that contains only the first mapped Entrez ID, in addition to the `entrez_ids` column that contains all mapped Entrez IDs per Ensembl ID.
