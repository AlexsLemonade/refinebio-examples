...
summary(mapped_df$Entrez)
```

There are 9088 NA's in our data frame, which means that 9088 out of the 31882 Ensembl IDs did not map to Entrez IDs.
...
Looks like we have some cases where 7 Entrez IDs mapped to a single Ensembl ID.
We have a total of 542 out of 31882 Ensembl IDs with multiple mappings to Entrez IDs.
With that rate, we can filter out the multi-mapped IDs if we want to keep 1:1 mappings for our downstream analyses by supplying the `"filter"` option to the `multiVals` argument og `mapIds()`, like we did in the [microarray gene identifier annotation example notebook](https://github.com/AlexsLemonade/refinebio-examples/tree/master/02-microarray/gene-id-annotation_microarray_01_ensembl.html).

However, we may also want to keep the multi-mapped information for our downstream analyses so let's go ahead and write our mapped results and data to file!
...
readr::write_tsv(multi_mapped_df, file.path(
  results_dir,
  "SRP056136_Entrez_IDs.tsv" # Replace with a relevant output file name
))
```
