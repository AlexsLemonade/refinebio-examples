...
Let's get ready to use the Ensembl IDs from our mouse dataset to obtain the associated Entrez IDs.
...
mapped_df <- mapped_list %>%
  reshape2::melt(value.name = "Entrez") %>%
...
multi_mapped_df <- mapped_df %>%
  ...
  # Create a new variable containing the number of Entrez IDs mapped to each ID
  dplyr::mutate(entrez_id_count = dplyr::n()) %>%
...
Looks like we have some cases where 15 Entrez IDs mapped to a single Ensembl ID.
...
We are not sure which Entrez ID is relevant as yet, but we could store all of the mapped information for future use.
...
```{r}
all_mapped_df <- df %>%
  # Collapse the mapped values in `mapped_list` into one column named `entrez_ids`
  dplyr::mutate(entrez_ids = sapply(mapped_list, paste, collapse = "-")) %>%
  # Select the `entrez_ids` column first, then everything else in the data frame
  dplyr::select(entrez_ids, dplyr::everything()) %>%
  # Now arrange by the new `entrez_ids` column
  dplyr::arrange(entrez_ids)
```

Let's take a look at our new collapsed `entrez_ids` column in the `all_mapped_df` object.

```{r}
all_mapped_df %>%
  # Filter `all_mapped_df` to include only the rows where `entrez_ids` values include the "-" character -- we know that these are the rows with multiple mapped values
  dplyr::filter(stringr::str_detect(entrez_ids, "-"))
```

We can store all of our mapped values in this format but we do not have a particular reason to store all or to choose one over the other, so let's just keep the first mapped Entrez ID that comes up.
We will do that in the next section by re-running the `mapIds()` function with `multiVals = "first"`.
