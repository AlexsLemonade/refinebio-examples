...
The purpose of this notebook is to provide an example of mapping gene IDs for RNA-seq data obtained from refine.bio using `AnnotationDbi` packages.
...
Go to this [dataset's page on refine.bio](https://www.refine.bio/experiments/SRP056136/rna-seq-of-control-and-foxo3b-mo-zebrafish-cells).
...
For this example analysis, we will use this [RNA-seq zebrafish dataset](https://www.refine.bio/experiments/SRP056136/rna-seq-of-control-and-foxo3b-mo-zebrafish-cells).

This dataset has 4 RNA-seq zebrafish samples.
...
Copy and paste the `SRP056136` folder into your newly created `data/` folder.
...
- A folder called "data" which contains:
  - The `SRP056136` folder which contains:
    - The gene expression  
    - The metadata TSV  
...
Although this example script uses Ensembl IDs from Zebrafish, (<i>Danio rerio</i>), to obtain Uniprot IDs this script can be easily converted for use with different species or annotation types <i>eg</i> protein IDs, gene ontology, accession numbers.
...
Let's get ready to use the Ensembl IDs from our mouse dataset to obtain the associated Uniprot IDs.
...
metadata <- readr::read_tsv(file.path(
  ...
  "metadata_SRP056136.tsv" # Replace with the name of your metadata file
))
# Read in data TSV file
df <- readr::read_tsv(file.path(
  data_dir, # Replace with path to your data file
  "SRP056136.tsv" # Replace with the name of your data file
)) %>%
...
```{r}
# Map Ensembl IDs to their associated Uniprot IDs
mapped_list <- mapIds(
  org.Dr.eg.db, # Replace with annotation package for the organism relevant to your data
  keys = df$Gene,
  column = "UNIPROT", # Replace with the type of gene identifiers you would like to map to
...
It looks like we have Uniprot IDs that were successfully mapped to the Ensembl IDs we provided.
...
mapped_df <- mapped_list %>%
  reshape2::melt(value.name = "Uniprot") %>%
...
We can see that our data frame has a new column `Uniprot`.
Let's get a summary of the values returned in the `Uniprot` column of our mapped data frame.

```{r}
# We can use the `summary()` function to get a better idea of the distribution of values in the `Uniprot` column
summary(mapped_df$Uniprot)
```

There are 13121 NA's in our data frame, which means that 13121 out of the 31882 Ensembl IDs did not map to Uniprot IDs.
This is not the best rate, in our opinion, which means that it may be wise to try a different gene identifier type that may have a better mapping rate.
...
multi_mapped_df <- mapped_df %>%
  # Let's group by the Ensembl ID's in the `Ensembl` column
  dplyr::group_by(Ensembl) %>%
  # Create a new variable containing the number of uniprot IDs mapped to each ID
  dplyr::mutate(uniprot_count = dplyr::n()) %>%
  # Arrange by the genes with the highest number of uniprot IDs mapped
  dplyr::arrange(desc(uniprot_count)) %>%
...
Looks like we have some cases where 8 Uniprot IDs mapped to a single Ensembl ID.
We have a total of 6732 out of 31882 Ensembl IDs with multiple mappings to Uniprot IDs.
We may want to keep this information for our downstream analyses.

Let's write our mapped results and data to file!

## Write mapped results to file

```{r}
# Write mapped and annotated data frame to output file
readr::write_tsv(multi_mapped_df, file.path(
  results_dir,
  "SRP056136_Uniprot_IDs.tsv" # Replace with a relevant output file name
))
```
