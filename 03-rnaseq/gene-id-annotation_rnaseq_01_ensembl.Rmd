...
Although this example script uses Ensembl IDs from Zebrafish, (<i>Danio rerio</i>), to obtain gene symbols this script can be easily converted for use with different species or annotation types <i>eg</i> protein IDs, gene ontology, accession numbers.
...
In the case of our [microarray gene identifier annotation example notebook](https://github.com/AlexsLemonade/refinebio-examples/tree/master/02-microarray/gene-id-annotation_microarray_01_ensembl.Rmd), a Mouse (<i>Mus musculus</i>) dataset is used, meaning `org.Mm.eg.db` or `Mm` would also need to be used there.
...
In this analysis, we will be using the `org.Dr.eg.db` R package [@Carlson2019].
...
There are 13121 NA's in our data frame, which means that 13121 out of the 31882 Ensembl IDs did not map to gene symbols.
This is not the best rate, which means that it may be wise to try a different gene identifier type that may have a better mapping rate.
...
Now, let's write our mapped results to file!
...
- See our [microarray gene ID conversion notebook](https://github.com/AlexsLemonade/refinebio-examples/tree/master/02-microarray/gene-id-annotation_microarray_01_ensembl.Rmd) as another applicable example, since the steps for this workflow do not change with technology [].
