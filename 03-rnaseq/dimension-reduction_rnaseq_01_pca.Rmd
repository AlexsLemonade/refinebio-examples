...
```{r}
# The `DESeqDataSetFromMatrix()` function will need all values as integers
df <- round(df)
```
...
dds <- DESeqDataSetFromMatrix(
  countData = df, # the counts values for all samples in our dataset
...
We want to filter out the genes that have not been expressed or that have low expression counts because we want to reduce noise in our data before we normalize  and create our plot.
We are going to do some pre-filtering to keep only genes with 10 or more reads total.
This threshold might vary depending on the number of samples and expression patterns acrosss your data set.
Note that rows represent gene data and the columns represent sample data in our dataset.

```{r}
# Filter `DESeqDataSet` object to include only genes
# with a total count of at least 10 reads across the samples
