...
We identified this module 19 as the most differentially expressed between our data's two time points (during illness and recovering from illness).
...
We will perform ORA on one of the modules we identified in the WGCNA analysis. 
In order to know which genes are a part of this module, we will import a saved results file that's stored online and contains the genes and what module they were identified to be a part of. 
...
In this example, we will focus on one module, module 19, which we previously identified as differentially expressed between our time points (during the acute bronchiolitis illness and during recovery from bronchiolitis). 
See [the previous section](#about-the-dataset-we-are-using-for-this-example) for more background on this gene list. 
