...
This notebook takes RNA-seq data and metadata from refine.bio and identifies differentially expressed genes. 
...
file.exists(file.path(data_dir, "SRP078441", "SRP078441.tsv"))

# Check if the metadata file is in the data directory stored as `data_dir`
file.exists(file.path(data_dir, "SRP078441", "metadata_SRP078441.tsv"))
```
...
head(dplyr::select(metadata_df, refinebio_title, mutation_status))
```
...
ggsave(plot = volcano_plot, 
       file.path(plots_dir,"SRP078441_volcano_plot.png")) # Replace with a plot name relevant to your data
```
