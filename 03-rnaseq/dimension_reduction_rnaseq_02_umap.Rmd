...
This notebook illustrates one way that you can use harmonized RNA-seq data from refine.bio to perform Uniform Manifold Approximation and Projection (UMAP) and plot the scores using `ggplot2`.
...
To run this example yourself, [download the `.Rmd` for this analysis by clicking this link](https://alexslemonade.github.io/refinebio-examples/03-rnaseq/dimension_reduction_rnaseq_02_umap.Rmd).
...
In this analysis, we will be using the R package [`DESeq2`](http://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html) [@DESeq2] for normalization, the R package [`umap`](https://www.rdocumentation.org/packages/umapr/versions/0.0.0.9001/topics/umap) [@UMAP] for the production of UMAP dimension reduction values and the R package , and the R package [`ggplot2`](http://r-statistics.co/Complete-Ggplot2-Tutorial-Part1-With-R-Code.html) [@ggplot2] for plotting the UMAP values.
...
Attach the `DESeq2`, `umap`, and `ggplot2` libraries:
...
metadata <- metadata %>%
  dplyr::select(
    ...
    refinebio_treatment,
    refinebio_disease
  ) %>% # The metadata has many variables, we want to select only those that we will need for plotting later
  dplyr::mutate(
    refinebio_treatment = as.factor(refinebio_treatment),
    refinebio_disease = as.factor(refinebio_disease)
  ) # Now let's make the annotation variable factors
```
...
```{r}
# We need to tell R to create a data frame with the umap counts and annotation data in preparation for plotting with `ggplot2`
...
In this code chunk, the variable `refinebio_treatment` is given to the `ggplot()` function as part of the goal of the experiment is to analyze the differential responses to androgen deprivation therapy (ADT).
...
ggplot(
  ...
  aes(
    x = X1,
    y = X2,
    color = refinebio_treatment # Here we are giving `refinebio_treatment` to the color argument
  )
) +
  geom_point() # This tells R that we want a scatterplot
```

In the next code chunk, the variables `refinebio_treatment` and `refinebio_disease` are given to `ggplot()` due to the hypothesis of the experiment that states that differences in the tumor transcriptional platforms may also show differences in response to the androgen deprivation therapy (ADT) as noted in the associated [paper](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6210624/) [@ADT-Prostate-Cancer].

Now let's plot using `ggplot2` functionality.

```{r}
# Plot using `ggplot()` function
final_annotatated_umap_plot <- ggplot(
  umap_plot_df,
  aes(
    x = X1,
    y = X2,
    color = refinebio_treatment,
    # Here we are giving `refinebio_treatment` to the color argument
    shape = refinebio_disease # Here we are giving `refinebio_disease` to the shape argument
  )
) +
  geom_point() # This tells R that we want a scatterplot

# Display the plot that we saved about
final_annotatated_umap_plot
```
...
ggsave(file.path(plots_dir, "SRP133573_umap_plot.png"), # Replace with name relevant your plotted data
  plot = final_annotatated_umap_plot
)
```

# Resources for further learning

- [Guidelines on choosing dimension reduction methods](https://journals.plos.org/ploscompbiol/article/file?id=10.1371/journal.pcbi.1006907&type=printable) [@dimension-reduction-guidelines]  
- [A nice explanation and comparison of many different dimenstionality reduction techniques that you may encounter](https://rpubs.com/Saskia/520216) [@dimension-reduction-explained]
