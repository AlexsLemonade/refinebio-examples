---
title: "Ortholog Mapping with `hcop` - RNA-seq"
...
This notebook demonstrates how you can use the [`hcop`](https://github.com/stephenturner/hcop) package to perform ortholog mapping for RNA-seq data obtained from refine.bio.
In this notebook, we will use mouse data from refine.bio and annotate it with human gene symbols from HGNC Comparison of Orthology Predictions (HCOP).
...
# Ortholog Mapping with `hcop` - RNA-seq
...
In this analysis, we will be using `hcop` to map between mouse and human identifiers [@Turner2018].
...
We'll use the `mouse` data from `hcop`; this is what `mouse` looks like.

```{r}
mouse
```

We're interested in the `human_symbol` and `mouse_ensembl` columns specifically.

```{r}
human_mouse_key <- mouse %>%
  # we'll want to subset mouse to only the columns we're interested in
  dplyr::select(mouse_ensembl, human_symbol)
...
We also have 18908 NAs, which is okay, because we do not expect everything to map across species.
...
Let's look at the `ENSMUSG00000000037` example below.

```{r}
human_mouse_mapped_df %>%
  dplyr::filter(Gene == "ENSMUSG00000000037") %>%
...
```{r}
collapsed_mouse_ensembl_df <- human_mouse_mapped_df %>%
  # Group by human gene symbols
  dplyr::group_by(human_symbol) %>%
  # Collapse the Ensembl IDs in `human_mouse_mapped_df` into one column named `all_mouse_ensembl_ids`
  dplyr::summarize(all_mouse_ensembl_ids = paste(Gene, collapse = ";"))
head(collapsed_mouse_ensembl_df)
```
...
```{r}
readr::write_tsv(
  collapsed_mouse_ensembl_df,
  file.path(
    results_dir,
    "SRP070849_collapsed_mouse_ensembl_to_human_gene_symbol_list.tsv" # Replace with a relevant output file name
  )
)
```
