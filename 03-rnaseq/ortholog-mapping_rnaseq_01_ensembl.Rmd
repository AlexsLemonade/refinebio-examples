...
Now copy and paste the `human_mouse_hcop_fifteen_column.txt.gz` file into your `data/` folder.
...
human_mouse_mapped_df <- df %>%
  ...
  # Sort so that the columns with the mouse Ensembl IDs, human Ensembl IDs and 
  # support databases are first
  dplyr::select(Gene, human_ensembl, support, dplyr::everything())
```
...
We have 18,801 NAs, which means we have 18,801 mouse Ensembl IDs that were not mapped to human Ensembl IDs.
...
Remember that our original data is associated to mouse Ensembl IDs, so when a human Ensembl ID maps to multiple mouse Ensembl IDs, the data will not get duplicated, but you will have multiple rows associated with that human Ensembl ID.
...
filtered_mouse_ensembl_df <- human_mouse_mapped_df %>%
  ...
  dplyr::group_by(human_ensembl) %>%
  # Now filter for the rows with more than one database in support for each human Ensembl ID
...
Now let's write our `filtered_mouse_ensembl_df` object, with the _reliable_ zebrafish Ensembl IDs for each unique human gene symbol, to file.
