...
mouse <-
  paste0(
    ...
    "human_",
    "mouse", # Replace with the organism you have data for
    "_hcop_fifteen_column.txt.gz"
  ) %>%
  data.table::fread() %>%
  set_names(names(.) %>% gsub("_gene$", "", .)) # Removing some extra wording in some of the column names
```
...
We'll use the `mouse` data from the HGNC database; this is what `mouse` looks like.
