...
## Mapping mouse Ensembl gene IDs to human symbols
...
We're interested in the `human_symbol`, `mouse_ensembl`, and `support` columns specifically.
...
human_mouse_key <- mouse %>%
  ...
  dplyr::select(mouse_ensembl, human_symbol, support)
...
human_mouse_mapped_df <- df %>%
  ...
  # Sort so that the columns with the mouse Ensembl IDs and human gene symbols are first
...
```{r}
# We can use the `count()` function to get a count of the distribution of values in the `human_symbol` column
human_mouse_mapped_df %>%
  dplyr::group_by(human_symbol) %>%
  dplyr::count() %>%
  # Sort by highest `n` which would be the human gene symbol with the most mapped mouse Ensembl IDs
  dplyr::arrange(desc(n))
```

Looks like we have mapped symbols!
Now, let's get an idea of how many mouse Ensembl IDs we have that were not mapped to human gene symbols.

```{r}
sum(is.na(human_mouse_mapped_df$human_symbol))
```

We have 18801 NAs, which means we have 18801 mouse Ensembl IDs that were not mapped to human gene symbols.
This is okay because we do not expect everything to map across species.

## Take a look at some multi-mappings

If a mouse Ensembl gene ID maps to multiple human symbols, the gene expression values get duplicated.
...
Remember that our original data is associated to mouse Ensembl IDs, so when a human gene symbol maps to multiple mouse Ensembl IDs, the data will not get duplicated, but you will have multiple rows associated with that human symbol.
Let's look at the `CACNA1B` example below.

```{r}
human_mouse_mapped_df %>%
  dplyr::filter(human_symbol == "CACNA1B")
```
...
In the next chunk, we show how we can collapse all the human gene symbols into one column where we store them all for each unique mouse Ensembl ID.

```{r}
collapsed_human_symbol_df <- human_mouse_mapped_df %>%
  # Group by Ensembl IDs
  dplyr::group_by(Gene) %>%
  # Collapse the mapped values in `human_mouse_mapped_df` into one column named `all_human_symbols`
...
Since multiple mouse Ensembl gene IDs map to the same human symbol, we can also collapse all Ensembl IDs that each unique gene symbol mapped to.
...
Therefore, we will not add the expression data here since the expression values are not the same across Ensembl IDs, despite mapping to the same human gene symbol.
...
filtered_mouse_ensembl_df <- human_mouse_mapped_df %>%
  ...
  dplyr::group_by(human_symbol) %>% # Group by human gene symbols
  dplyr::filter(n_databases == max(n_databases)) # Now filter for the row with the largest number of databases in support for each human gene symbol
...
Now let's write our list of collapsed mouse Ensembl IDs for each unique human gene symbol to file.
