...
This notebook illustrates one way that you can use harmonized RNA-seq data from refine.bio to perform Principal Component Analysis (PCA) and plot the scores using `ggplot2`. 
...
To run this example yourself, [download the `.Rmd` for this analysis by clicking this link](https://alexslemonade.github.io/refinebio-examples/03-rnaseq/dimension_reduction_rnaseq_01_pca.Rmd).
...
The data that we downloaded from refine.bio for this analysis has 175 samples (obtained from 20 patients with prostate cancer), containing RNA-sequencing results for samples that have been treated with ADT obtained via radical prostatectomy and that have not been treated with ADT obtained via biopsy (this was done for each patient).
...
In this analysis, we will be using the R package [`DESeq2`](http://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html) [@DESeq2] for normalization and production of PCA values and the R package [`ggplot2`](http://r-statistics.co/Complete-Ggplot2-Tutorial-Part1-With-R-Code.html) [@ggplot2] for plotting the PCA values.
...
Attach the `ggplot2` and `DESeq2` libraries:
...
Now we are going to use a combination of functions from the `DESeq2` and `ggplot2` packages to look at the differential expression of our pre-ADT and post-ADT samples.
...
metadata$refinebio_treatment <- as.factor(metadata$refinebio_treatment)
metadata$disease <- as.factor(metadata$refinebio_disease)
...
In this code chunk, the variable `refinebio_treatment` is given to the `plotPCA()` function as part of the goal of the experiment is to analyze the differential responses to androgen deprivation therapy (ADT).
...
In this chunk, we are going to add a variable to our plot for annotation.

The variables `refinebio_treatment` and `refinebio_disease` are given to the `plotPCA()`
due to the hypothesis of the experiment stating "that differences in tumor transcriptional programs may reflect differential responses to androgen deprivation therapy (ADT) and subsequent metastasis".
...
It's hard to distinguish the different `refinebio_treatment` versus `refinebio_disease` in the plot above.

Let's use the `ggplot2` package functionality to customize our plot further.
...
# Now let's plot using `ggplot2` functionality
ggplot(pcaData, aes(
  x = PC1,
  y = PC2,
...
png(file.path(
  plots_dir,
  "SRP133573_annotated_pca_plot.png" # Replace file name with a relevant output plot name
))

# Paste the code to create your PCA plot from the chunk above
ggplot(pcaData, aes(
  x = PC1,
  y = PC2,
  color = refinebio_treatment,
