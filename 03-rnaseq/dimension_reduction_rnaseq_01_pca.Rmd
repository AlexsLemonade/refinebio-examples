...
due to the hypothesis of the experiment that states that differences in the tumor transcriptional platforms may also show differences in response to the androgen deprivation therapy (ADT) as noted in the associated [paper](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6210624/) [@ADT-Prostate-Cancer].

```{r}
plotPCA(dds_norm,
  intgroup = c("refinebio_treatment", "refinebio_disease") # Note that we are able to add another variable to the intgroup argument here using the `c()` function
)
```
...
ggsave(file.path(plots_dir, "SRP133573_pca_plot.png"), # Replace with name relevant your plotted data
  plot = last_plot() # This tells R to save the last plot that was modified or created
)
```

# Resources for further learning

- [Principle Component Analysis (PCA) Explained Visually](http://setosa.io/ev/principal-component-analysis/) [@pca-explained]
