...
For this example analysis, we will use this [acute myeloid leukemia (AML) dataset](https://www.refine.bio/experiments/SRP123625) [@Micol2017]
...
metadata <- metadata %>%
  ...
  dplyr::mutate(mutation_status = dplyr::case_when(
    stringr::str_detect("R98S", mutation_status) ~ "R98S",
    stringr::str_detect("WT", mutation_status) ~ "wildtype"
  ))
```
...
Currently, `mutation_status` is a character.
...
metadata <- metadata %>%
  dplyr::mutate(
    ...
    mutation_status = factor(mutation_status, levels = c("no_mutation", "asxl_mutation"))
  )
```
...
```{r}
plotCounts(ddset, gene = "ENSG00000196074", intgroup = "mutation_status")
```
