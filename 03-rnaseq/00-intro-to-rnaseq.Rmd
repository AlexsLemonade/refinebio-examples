...
Although DESeq2 has multiple transformation methods, we generally stick to `vst()` (Variance Stablizing Transformation) or `rlog()`. 
...
Comparing `rlog()` and `vst()`, `rlog()` sometimes takes a bit longer to run [@Love2019].
