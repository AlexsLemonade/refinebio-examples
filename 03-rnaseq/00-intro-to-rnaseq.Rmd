...
To normalize and transform our data with DESeq2, we generally use `vst()` (Variance Stablizing Transformation) or `rlog()`. 
...
Between `rlog()` and `vst()`, they are quite similar, but `rlog()` takes a bit longer to run [@Love2019].
If you end up using a larger dataset and `rlog()` transformation takes a bit too long, you can switch to using `vst()` with confidence since previous data shows they yield similar results given the dataset is large enough [@Love2019]. 
