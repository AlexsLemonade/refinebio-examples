...
author.full.results <- readr::read_tsv(file.path(results_dir, 
...
author.full.results <- author.full.results %>%
  # You may need to replace "affy_probe_ids" with the name of the column that        # contains the probe IDs in your data 
  ...
  # You may need to replace <logFC> with the name of the column that contains the    # log fold change values in your data 
...
direction.summary <- author.full.results %>% 
  # Keep only significant at 0.05 probes -- you may need to replace <adj.P.Val>      # with the name of the column that contains the adjusted P values for your data
...
refine.bio.data.filename <- file.path(data_dir,
...
```{r}
# Metadata file name
metadata.filename <- file.path(data_dir,
...
```{r  Write results}
readr::write_tsv(refine.bio.results, file.path(results_dir,
...
up.plot.file <- file.path(plots_dir, 
...
```{r}
# Make filename to save plot as
down.plot.file <- file.path(plots_dir, 
