...
This is a more intuitive approach to gene network analysis which is pretty widely used for microarray (as well as RNA-seq). 

As output, WGCNA gives groups of co-expressed genes as well as an eigengene x sample matrix (where eigengenes are [eigenvalues](https://www.khanacademy.org/math/linear-algebra/alternate-bases/eigen-everything/v/linear-algebra-introduction-to-eigenvalues-and-eigenvectors) but are the collapsed information for a group of co-expressed genes). 
...
If you end up wanting to run WGCNA with a microarray dataset, the normalization done by refine.bio _should_ be sufficient, but you will likely want to [apply a minimum expressiion filter](#define-a-minimum-counts-cutoff) as we do in this example. 
