...
Now we can find what genes are a part of module 52 and make it into a vector. 
...
mod52_df <- df %>% 
  ...
  dplyr::filter(Gene %in% module_52_genes) %>% 
  tibble::column_to_rownames("Gene")
...
```{r}
heatmap <- pheatmap::pheatmap(as.matrix(mod52_df), 
...
```{r}
sample_order <- heatmap$tree_col$labels

barplot <- ggplot(annot_df[sample_order, ], aes(x = rownames(annot_df), 
                                y = module_52_eigengene, 
                                fill = refinebio_treatment)) + 
  geom_bar(position = "dodge", stat = "identity") +
  theme_classic() +
  theme(axis.text.x = element_blank(), 
        legend.position = "none") + 
  ggplot2::scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  xlab("refinebio_accession_code")
...
```{r}
multi_panel_plot <- gridExtra::grid.arrange(grobs = list(heatmap[[4]], barplot), ncol = 1, nrow = 2)
```

```{r}
ggsave(plot = multi_panel_plot, file = file.path("results", "SRP133573_module_52_eigengene_heatmap.png"))
```
