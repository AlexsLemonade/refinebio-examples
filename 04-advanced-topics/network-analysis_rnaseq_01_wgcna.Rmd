...
Now we can find what genes are a part of module 52 and 6and make it into a vector. 
...
We will make a multi-panel plot that summarizes our differentially expressed module.

```{r}
col_annot_df <- metadata %>% 
  dplyr::select(refinebio_accession_code, refinebio_treatment) %>%
  dplyr::mutate(module_52_eigengene = module_eigengenes$ME52) %>% 
  tibble::column_to_rownames("refinebio_accession_code") %>% 
  dplyr::arrange(refinebio_treatment)

row_annot_df <- module_52_genes %>% 
  tibble::column_to_rownames("gene") %>% 
...
mod52_df <- df %>% 
  ...
  dplyr::filter(Gene %in% module_52_6_genes$gene) %>% 
  tibble::column_to_rownames("Gene") %>% 
  dplyr::select(rownames(col_annot_df)) 

mod52_df <- mod52_6_df[rownames(row_annot_df), ]  

mod52_mat <- as.matrix(mod52_df) 

mod52_mat_scaled <- t(apply(mod52_mat, 1, scale))
```
...
color_func <- circlize::colorRamp2(c(-1, 0, 1), 
 ...
 # Create the Heatmap annotation object
  col_annot <- ComplexHeatmap::HeatmapAnnotation(
    df = data.frame(col_annot_df %>% dplyr::select(refinebio_treatment)),
    module_52 = ComplexHeatmap::anno_barplot(col_annot_df$module_52_eigengene), 
    col = list("post-adt" = "#f1a340", "pre-adt" = "#998ec3")
  )

  # Plot on a heatmap
  heatmap <- ComplexHeatmap::Heatmap(mod52_mat_scaled,
    col = color_func,
...
```{r}
ggsave(plot = heatmap, file = file.path("results", "SRP133573_module_52_and_6_eigengene_heatmap.png"))
```
