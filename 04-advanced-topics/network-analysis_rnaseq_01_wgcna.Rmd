...
As output, WGCNA gives groups of co-expressed genes as well as an eigengene x sample matrix (where eigengenes are [eigenvalues](https://www.khanacademy.org/math/linear-algebra/alternate-bases/eigen-everything/v/linear-algebra-introduction-to-eigenvalues-and-eigenvectors) but are the collapsed information for a group of co-expressed genes) [@KhanAcademy; @Langfelder2007]. 
This eigengene x sample data can, in many instances, used as you would the original gene expression values. 
...
However, note that very large datasets will be difficult to run locally (on a personal laptop).
...
But WGCNA also requires a package called `impute` that is sometimes has trouble installing so we recommend installing that first. 
