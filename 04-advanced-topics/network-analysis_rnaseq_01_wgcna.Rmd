...
```{r}
module_52_6_genes <- gene_modules_df %>%
  dplyr::filter(module == "ME52" | module == "ME6")
```
...
col_annot_df <- metadata %>% 
  ...
  dplyr::mutate(module_52_eigengene = module_eigengenes$ME52, 
                module_6_eigengene = module_eigengenes$ME6) %>% 
  tibble::column_to_rownames("refinebio_accession_code") %>% 
  dplyr::arrange(refinebio_treatment)

row_annot_df <- module_52_6_genes %>% 
  tibble::column_to_rownames("gene") %>% 
...
mod52_6_df <- mod52_6_df[rownames(row_annot_df), ]  
```


```{r}
heatmap <- pheatmap::pheatmap(as.matrix(mod52_6_df), 
