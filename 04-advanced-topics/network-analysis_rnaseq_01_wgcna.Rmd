...
make_module_heatmap <- function(module_name = "ME52", 
  ...
  if (standardize_genes) {
    ...
    color_func <- circlize::colorRamp2(c(-1, 0, 1), 
                                     c("#ef8a62", "#f7f7f7", "#67a9cf"))
  } else {
    # Create a color function based on standardized scale
    color_func <- circlize::colorRamp2(c(quantile(mod_mat, 0.05), 
                                       median(mod_mat), 
  ...
  heatmap <- ComplexHeatmap::Heatmap(mod_mat,
    ...
    # We don't want to cluster genes or samplese
...
We can save this plot by usiing `ggsave()`. 

```{r}
ggsave(plot = mod_52_heatmap, file = file.path("results", "SRP133573_module_52_heatmap.png"))
```
