...
We need this to be a factor, but we also need `pre-adt` as the first level. 
...
make_module_heatmap <- function(module_name,
  ...
  if (standardize_genes) {
    # Genes are on very different scales; we will normalize (Z score) them
    mod_mat <- mod_mat %>%
    ...
    # Create a color function based on observed scale
    color_func <- circlize::colorRamp2(
      c(
        quantile(mod_mat, 0.025),
        median(mod_mat),
        quantile(mod_mat, 0.975)
      ),
      c("#67a9cf", "#f7f7f7", "#ef8a62")
    )
  }
...
In general expression is pretty low for all the genes that make up module 10 but there's a few samples and some genes that show higher expression. 
