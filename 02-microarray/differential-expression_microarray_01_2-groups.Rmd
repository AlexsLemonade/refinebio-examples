...
@Tregnago2016 use microarrays to measure gene expression of zebrafish samples overexpressing human CREB, as well as control samples.
...
To do this, we will use a function from the `forcats` package, simplifying `"overexpressing the human CREB"` to just `CREB`.
...
metadata <- metadata %>%
  ...
  # change the name of the CREB genotype (and make genotype a factor)
  dplyr::mutate(
    genotype = forcats::fct_recode(genotype,
      CREB = "overexpressing the human CREB"
    )
  )
```
...
After fitting our data to the linear model, in this example we apply empirical Bayes smoothing and Benjamini-Hochberg multiple testing correction.
