...
### Medulloblastoma WNT subtype ORA

Let's generate a list of genes that are indicative of the WNT subtype in medulloblastoma data.
...
# cell types
top_wnt_genes <- dge_annot_df %>%
  # Here we want the 100 smallest p values which will be the biggest changes
  dplyr::top_n(WNTvsOther, n = -100) %>%
...
top_wnt_genes <- unique(as.character(top_wnt_genes))
...
kegg_ora_results <- enricher(
  gene = top_wnt_genes, # Genes of interest
  pvalueCutoff = 0.1,
