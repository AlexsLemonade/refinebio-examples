...
The original refine.bio dataset used to obtain these results is this [medulloblastoma subtype dataset](https://www.refine.bio/experiments/GSE37418/novel-mutations-target-distinct-subgroups-of-medulloblastoma) [@Robsinson2012]. 
...
The `WebGestaltR` function is a wrapper for the [WEB-based GEne SeT AnaLysis Toolkit (WebGestalt)](http://www.webgestalt.org/) [@Wang2017].
...
results_df <- readr::read_tsv(
  file.path(
    results_dir,
    "Project_GSE37418_up_SHH_lfc2_fdr0_05", # This will need to change depending on what you supply to `projectName` argument in `WebGestaltR()` function
    "enrichment_results_GSE37418_up_SHH_lfc2_fdr0_05.txt" # Same here, but the prefix is different
  )
)
```
...
```{r}
head(as.data.frame(results_df)) # We are coercing to a data frame (from a tibble) so all results are rendered a bit more clearly
```
It looks like some of the top results are pathways associated with ribosomes and translation.
...
- [WebGestalt 2019 Manual](http://www.webgestalt.org/WebGestalt_2019_Manual.pdf) [@Wang2019].
- [`WebGestaltR` package documentation](https://www.rdocumentation.org/packages/WebGestaltR/versions/0.3.0) [@Wang2017].
