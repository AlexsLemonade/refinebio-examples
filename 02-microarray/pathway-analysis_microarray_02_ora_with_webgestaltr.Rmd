...
In this example, we are using differential expression results from an [example analysis of medulloblastoma subgroups](https://alexslemonade.github.io/refinebio-examples/differential-expression_microarray_02_several-groups.html) using [`limma`](https://bioconductor.org/packages/release/bioc/html/limma.html) [@Ritchie2015].
...
We are using `head()` so we just get a preview and not the whole table printed out. 
...
Let's take a look at one of the results tables look like by reading it into R. 
It is a `.txt` file, however its values are separated by tabs, making it a TSV; hence we can read it in with `readr::read_tsv()` function. 

```{r}
results_df <- readr::read_tsv(
  file.path(
    results_dir,
...
Let's print out a preview of it.   
