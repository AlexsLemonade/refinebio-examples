...
```{r}
readr::write_tsv(symbol_exprs_df, file.path(data_dir,
...
```{r}
grep("ECM_RECEPTOR", # Replace with an exact word or phrase that would filter in your desired pathway(s) as the `grep` function looks for all instances of the pattern given -- names of pathways can be seen using the `names(kegg_pathways)` function
     names(kegg_pathways))
```
