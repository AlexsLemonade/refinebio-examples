...
For this example analysis, we will use this [diffuse large B-cell lymphoma dataset](https://www.refine.bio/experiments/GSE45495).
...
Copy and paste the `GSE45495` folder into your newly created `data/` folder.
...
- A folder called "data" which contains:
  - The `GSE45495` folder which contains:
    - The gene expression  
    - The metadata TSV  
...
In this analysis, we will be using the R package `pheatmap` [@Slowikowski2017] for clustering and creating a heatmap.
...
metadata <- readr::read_tsv(file.path(
  ...
  "metadata_GSE45495.tsv" # Replace with the name of your metadata file
))

# Read in data TSV file
df <- readr::read_tsv(file.path(
  data_dir, # Replace with path to your data file
  "GSE45495.tsv" # Replace with the name of your data file
)) %>%
...
png(file.path(
  plots_dir,
  "heatmap_GSE45495_non_annotated.png" # Replace file name with a relevant output plot name
))
...
From the accompanying [paper](https://pubmed.ncbi.nlm.nih.gov/23840064/), we know that the some samples were treated with PTEN cDNA for varied durations of time and others were not treated with PTEN cDNA.
We are going to annotate our heatmap with the variables that hold the treatment data and the recorded duration of time for the specified treatment for each sample.
...
png(file.path(
  plots_dir,
  "heatmap_GSE45495_annotated.png" # Replace file name with a relevant output plot name
))
