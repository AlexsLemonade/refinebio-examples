...
In the next chunk, we have adapted code from the [`hcop` package R script](https://github.com/stephenturner/hcop/blob/master/data-raw/hcop.R) to retrieve the zebrafish and human mapped data from the HGNC database [@Turner2018].

```{r}
# Define URL to desired organism ortholog mapping file from the HGNC database
zebrafish_hgnc_url <- paste0(
  "ftp://ftp.ebi.ac.uk/pub/databases/genenames/hcop/",
  "human_",
  "zebrafish",
  # Replace with the organism you have data for; see the list of possible files from the HGNC at this link: ftp://ftp.ebi.ac.uk/pub/databases/genenames/hcop/
...
zebrafish <- data.table::fread(zebrafish_hgnc_url) %>% # Here we use `data.table::fread()` instead of `readr::read_tsv()` because we expect this file to be large so the `fread()` function is faster/more efficient here
  set_names(names(.) %>%
...
We have 464 NAs, which means we have 464 zebrafish Ensembl IDs that were not mapped to human gene symbols.
...
Therefore, where we have multi-mapped zebrafish Ensembl gene IDs, we will take the mapping with the most databases that concur according to the `support` column.
...
Now let's write our list of filtered zebrafish Ensembl IDs for each unique human gene symbol to file.
...
readr::write_tsv(
  ...
  file.path(
    results_dir,
    "GSE71270_filtered_zebrafish_ensembl_to_human_gene_symbol_list.tsv" # Replace with a relevant output file name
  )
)
```
