...
human_zebrafish_key <- human_zebrafish_key %>%
  # We need to use the `distinct()` function to remove duplicates resulting from
...
If a zebrafish Ensembl gene ID maps to multiple human symbols, the associated Ensemble ID values will get duplicated in our output data.
...
We can see that we have multiple zebrafish Ensembl IDs that mapped to the same gene, but we also still have some duplicate Ensembl IDs here because the `support` column was different in the original data set!
...
collapsed_human_symbol_df <- human_zebrafish_mapped_df %>%
  ...
  dplyr::group_by(Gene) %>%
  # Collapse the mapped values in `human_zebrafish_mapped_df` to one column named
  # `all_human_symbols`, removing any duplicate human symbols
...
Simply combining expression values across zebrafish transcripts using an average or other summary statistic may result in the loss of a lot of data and will likely not be representative of the zebrafish biology.
...
human_zebrafish_mapped_df %>%
  ...
  dplyr::filter(n > 1)
```
Looks like we have 4,211 human gene symbols with multiple mappings.
...
filtered_zebrafish_ensembl_df %>%
  ...
  dplyr::filter(n > 1)
```
Now we only have 1,717 multi-mapped genes, compared to the 4,211 that we began with.
