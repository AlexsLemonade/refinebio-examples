...
For this example analysis, we will use this [CREB overexpression zebrafish dataset](https://www.refine.bio/experiments/GSE71270/creb-overexpression-induces-leukemia-in-zebrafish-by-blocking-myeloid-differentiation-process), which contains 10 samples.
...
readr::write_tsv(
  ...
  file.path(
    results_dir,
    "GSE71270_ensembl_to_human_gene_symbol.tsv" # Replace with a relevant output file name
  )
)
```
...
One way to resolve this may be to map to a different gene identifier.
...
```{r}
collapsed_ensembl_df <- human_symbol_df %>%
  # Group by human gene symbols
  dplyr::group_by(human_symbol) %>%
  # Collapse the Ensembl IDs in `human_symbol_df` into one column named `all_zebrafish_ensembl_ids`
  dplyr::summarize(all_zebrafish_ensembl_ids = paste(zebrafish_ensembl, collapse = ";"))

head(collapsed_ensembl_df)
```
The final format in which you save your results data frame largely depends on your question and the downstream analyses/tools that you would like to implement.

## Write collapsed results to file

We are going to save the results data frame where we have collapsed human gene symbols into one column for each unique zebrafish Ensembl ID.
...
readr::write_tsv(
  ...
  file.path(
    results_dir,
    "GSE71270_collapsed_ensembl_to_human_gene_symbol.tsv" # Replace with a relevant output file name
  )
)
```
