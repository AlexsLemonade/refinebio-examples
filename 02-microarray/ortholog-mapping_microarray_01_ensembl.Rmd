...
Enembl gene identifiers have different species-specific prefixes. 
...
# Now let's join the expression data from `df` for the subset of genes in `df`
...
We also have 415 NAs, which is okay, because we do not expect everything to map across species.
...
Using another gene identifier may result in a better 1:1 ratio to the zebrafish Ensembl IDs, i.e. human Ensembl IDs instead of human gene symbols.
...
* On the other, if a human gene symbol maps to multiple zebrafish Ensembl IDs, the gene expression values do not necessarily get duplicated.
...
## Collapse multiple mapped human gene symbols
...
## Collapse multiple mapped zebrafish Ensembl IDs
