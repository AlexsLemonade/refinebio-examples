...
## Sort out multi-mappings

Multiple zebrafish Ensembl gene IDs map to the same human symbol which results in duplicated human gene symbols.
Some downstream tools may require the data be simplified to 1:1 mappings.
Using another gene identifier may result in a better 1:1 ratio to the zebrafish Ensembl IDs, _i.e._ human Ensembl IDs instead of human gene symbols.
See our [gene ID annotation microarray notebook](https://alexslemonade.github.io/refinebio-examples/02-microarrary/gene-id-annotation_microarray_01_ensembl.html) for an example of mapping between gene identifiers.
...
Remember that our original data is associate to zebrafish Ensembl IDs, so when a human gene symbol maps to multiple zebrafish Ensembl IDs, the data will not get duplicated, but you will have multiple rows associated with that human symbol.
