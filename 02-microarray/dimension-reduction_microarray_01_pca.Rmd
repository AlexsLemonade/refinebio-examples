...
In this code chunk, we are going to perform Principal Component Analysis (PCA) on our data and create a data frame using the 

```{r}
# Perform Principal Component Analysis (PCA)
pca <- prcomp(t(df)) # We have to transpose our data frame to meet the `prcomp()` function requirements

# Make into data frame for plotting PC1 and PC2 with `ggplot2`
...
Let's plot the PC scores for the first two principal components.
...
- [Overall Explanation by Matt Brems](https://towardsdatascience.com/a-one-stop-shop-for-principal-component-analysis-5582fb7e0a9c) [@Brems2017] 
