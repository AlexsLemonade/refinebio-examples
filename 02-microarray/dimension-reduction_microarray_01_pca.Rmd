...
pca <- prcomp(t(df), # We have to transpose our data frame to meet the `prcomp()` function requirements
  scale = TRUE # This tells R
)

# Make the first 10 principal components into a data frame for plotting with `ggplot2`
...
By accessing the `importance` element, which contains the proportion of variance explained by each principal component, with `<summary results>$importance`, we can use indexing to only look at the first `n` PCs.

```{r}
# Save the summary of the PCA results using the `summary()` function
pca_summary <- summary(pca)

# Now access and store the importance information for the first 10 PCs
pca_importance <- pca_summary$importance[, 1:10]

# Let's take a look at the importance information
pca_importance
```

Now that we've seen the proportion of variance for the first ten PCs, let's plot the PC scores for the first two principal components, the components responsible for the most explained proportion of variance in our dataset. 

Let's also label the data points based on their genotype subgroup since this is central to the subgroup specific based hypothesis in the [original paper](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3683624/#SD1) [@Northcott2012].
...
We can add another label to our plot.
