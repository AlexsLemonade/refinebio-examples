...
We are using a `+ 0` in the model so the first group is not treated as your intercept. 
...
Now we are ready to actually start setting up our differential expression model. 
...
We're dividing by three in this expression so that each is contrasted to the average of the other three groups (`makeContrasts()` doesn't allow you to use functions like `mean()`, it wants a formula). 
...
We highly recommend consult the [limma users guide](https://bioconductor.org/packages/release/bioc/vignettes/limma/inst/doc/usersguide.pdf) for figuring out what your `makeContrasts()` and `model.matrix()` set ups should look like [@Ritchie2015]. 
