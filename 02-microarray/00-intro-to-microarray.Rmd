...
There are many different kinds of microarray platforms. 

Microarrays can be categorized by their probe chemistry and manufacturer: [Affymetrix GeneChips](https://www.thermofisher.com/us/en/home/life-science/microarray-analysis/transcriptome-profiling-microarrays.html), [Illumina Beadarray](https://www.illumina.com/documents/products/datasheets/datasheet_gene_exp_analysis.pdf); all of which supported on refine.bio.
But, refine.bio only supports single-color arrays, although there are [two-color arrays](https://www.ebi.ac.uk/training/online/course/functional-genomics-ii-common-technologies-and-data-analysis-methods/microarrays), so our examples and advice are generally from the perspective of using single-color array. 
...
- Microarray often has a faster turn-around than RNA-seq [@LCSciences2014].

### Microarray data **limitations**:  

- If a transcript doesn't have a probe designed to it on a microarray, it won't be measured; microarray can't be used for transcript discovery [@Mantione2014]. 
...
Different microarray chips, are also a type of batch effect, but quantile normalizations allows us to compare data from different chips that to a limited degree if we proceed with caution. 
...
Refine.bio uses [Brainarray packages](http://brainarray.mbni.med.umich.edu/Brainarray/Database/CustomCDF/) to annotate the microarray probe data [@Dai2005]. 
...
This also identifies which probes may have since become obsolete (they do not bind reliably to one location according to updated genome annotations). 
If your gene of interest was covered by the original probes of the microarray chip and the version of the Brainarray package used does maintains that it is still accurate, your gene of interest will show up in the Gene column.
