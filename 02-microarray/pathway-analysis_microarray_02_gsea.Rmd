...
Specifically, genes are ranked from most highly positive to most highly negative based on their statistic and a running sum is calculated by starting with the most highly ranked genes and increasing the score when a gene is in the pathway and decreasing the score when a gene is not.
In this example, the ES for a pathway is the running sum's maximum deviation from zero.
...
This example uses GSEA pre-ranked, which is a way of running GSEA when you have previously computed gene-level statistics.
...
GSEA requires _unique gene identifiers_ to produce the most accurate results, so we will need to resolve any duplicates found in our dataset.
...
As we mentioned earlier, we will want to remove duplicated gene identifiers in preparation for the GSEA steps later. 
Let's keep the Entrez IDs associated with the higher absolute value of the t-statistic.
GSEA relies on genes' rankings on the basis of a gene-level statistic and the ES that is calculated reflects the degree to which genes in a gene set are overrepresented in the top or bottom of the rankings [@clusterProfiler-book].
...
A negative enrichment score will be returned when many genes are near the bottom of the ranked list.
