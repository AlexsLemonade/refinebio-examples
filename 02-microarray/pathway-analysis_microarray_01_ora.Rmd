...
The file we use here has several columns of differential expression summary statistics. 
If you'd like to adapt an example analysis to use a different dataset from [refine.bio](https://www.refine.bio/), we recommend replacing the `dge_url` with a different file path to a read in a similar table of genes with the information that you are interested in. 
...
We will also declare a file path to where we want this filie to be downloaded to and we can use the same file path later for reading the file into R. 
...
Note you can also supply `dge_url` directly to the `readr::read_tsv()` function and read in the file without downloadnig it. 
