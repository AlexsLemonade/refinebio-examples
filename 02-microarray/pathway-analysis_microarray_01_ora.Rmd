...
ORA is a broadly applicable technique that may be good in scenarios where the other pathway analyses methods don't fit the dataset requirements or scientific questions you have. 
...
If you'd like to adapt an example analysis to use a different dataset from [refine.bio](https://www.refine.bio/), we recommend replacing the `dge_url` with a different file that contains a gene list that you are interested in. 
...
As far as code, we still suggest saving plots and results to `plots/` and `results/` directories, respectively, as these are automatically created by the notebook.
But, from here you can customize this ORA example to fit your own scientific questions and preferences.
...
For ORA, we only need a list of gene IDs as our input. 
There's a lot of different scenarios which you may end up with a gene list you want to know more about what biological pathways it coincides with. 

For this example, we will read in the differential expression results we will download from online.
...
Generally, this dataset would be a good candidate for [GSEA](https://alexslemonade.github.io/refinebio-examples/02-microarray/pathway-analysis_microarray_02_gsea.html) since it has gene-level rankings, but here we are showing you how to use ORA since it can be useful in a number of other situations. 
