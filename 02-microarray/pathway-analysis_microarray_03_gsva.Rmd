...
Note that using the maximum expression value to handle the duplicate Entrez gene identifiers means preferentially choosing values of a higher rank for use in the downstream GSVA steps.
However, this is just one of the expression values that can be used to handle duplicate gene identifiers.
...
Also keep in mind that these values are ultimately representing probe identifiers for our microarray dataset, as opposed to transcript reads usually associated with RNA-seq data.

There are two gene identifiers here for which we need to select the maximum expression values per sample across the duplicate Entrez gene identifier instances, so it is unlikely to have a considerable impact on our results.

Let's try the selection the maximum expression values method for one of the samples.
First, let's isolate a single sample along with the duplicate gene identifiers.

```{r}
single_sample_max_dup_df <- mapped_df %>%
  # Select the column with the Entrez gene identifiers along with one of the
  # samples/columns -- we are going to select the third column here since we
  # know the first two columns contain gene identifiers
  dplyr::select(entrez_id, 3) %>%
  # Filter to include only the rows associated with the duplicate Entrez gene identifiers
  dplyr::filter(entrez_id %in% dup_entrez_ids)

single_sample_max_dup_df
```

We've isolated one of the samples and its expression values for each of the instances of a duplicate Entrez gene identifier.
Now let's use the `summarize_all()` function to get the maximum expression value for each unique Entrez gene identifier.

```{r}
single_sample_max_dup_df <- single_sample_max_dup_df %>%
  # First, we need to group by Entrez IDs
...
single_sample_max_dup_df
```
Great, looks like we were able to obtain the single sample's maximum expression values for each Entrez gene identifier!

Let's apply this method to all of the samples.
...
Looks like we were able to successfully get rid of the duplicate Entrez gene identifiers by keeping the maximum expression value per sample associated with each of the duplicate gene identifiers!

Now let's combine the rest of the mapped data!
...
filtered_mapped_df <- mapped_df %>%
  ...
  # Now let's remove the column we no longer need
  dplyr::select(-"Ensembl")
```

Let's preview `filtered_mapped_df` to check that we combined the data associated with the duplicate Entrez IDs in `max_dup_df` with the rest of the mapped data.

```{r}
# Filter our object display the observations associated with the Entrez IDs in `dup_entrez_ids`
filtered_mapped_df %>%
  dplyr::filter(entrez_id %in% dup_entrez_ids)

# Preview the first six observations of `filtered_mapped_df` to ensure that we combined the rest
# of the mapped data associated with the Entrez gene identifiers that were not duplicated
head(filtered_mapped_df)
```

Great, looks like we were able to successfully handle the duplicate Entrez IDs by keeping the maximum expression value per sample, and combine this data with the rest of the mapped data!
