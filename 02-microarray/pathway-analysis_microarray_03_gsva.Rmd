...
We're dividing by three in this expression so that each group is compared to the average of the other two groups (`makeContrasts()` doesn't allow you to use functions like `mean()`; it wants a formula).
...
Let's make a sina plot to look at the differences among subgroups using our GSVA scores for `HALLMARK_UNFOLDED_PROTEIN_RESPONSE`.
...
annotated_results_df <-
  ...
  # Make this into a tidy data frame format
...
sina_plot <-
  ggplot(
    annotated_results_df, # Supply our annotated data frame
    aes(
      x = subgroup, # Replace with a grouping variable relevant to your data
      y = HALLMARK_UNFOLDED_PROTEIN_RESPONSE, # Our pathway of interest
