...
As we mentioned earlier, we will want to collapse any duplicate gene identifiers in preparation for the GSVA steps later.
GSVA is executed on a per sample basis so let's keep the maximum expression value per sample associated with the duplicate gene identifiers.
Note that using the maximum expression value to collapse duplicate gene identifiers means preferentially choosing a value or group of a higher rank downstream.
However, this is just one of the expression values that can be used to collapse duplicate gene identifiers.
...
We are collapsing the values for two gene identifiers here, so it is unlikely to have a considerable impact on our results.

```{r}
max_dup_df <- mapped_df %>%
  # Filter to include only the rows associated with the duplicate gene identifiers
  dplyr::filter(entrez_id %in% dup_entrez_ids) %>%
  # Group by Entrez IDs
  dplyr::group_by(entrez_id) %>%
  # Get the maximum expression value using all values associated with the grouping variable
  dplyr::summarize_all(max)

max_dup_df
```
Looks like we were able to successfully collapse the duplicate gene identifiers by keeping the maximum expression value per sample associated with each of the duplicate gene identifiers!

Now let's combine the rest of the mapped data!

```{r}
filtered_mapped_df <- mapped_df %>%
  # First let's get the data associated with the gene identifiers that aren't duplicated
  dplyr::filter(!entrez_id %in% dup_entrez_ids) %>%
  # Now let's bind the rows of the data we collapsed in `max_dup_df`
  dplyr::bind_rows(max_dup_df) %>%
  # Arrange so that the Entrez IDs that were duplicated are at the top
  dplyr::arrange(dplyr::desc(entrez_id %in% dup_entrez_ids)) %>%
  # Now let's remove the column we no longer need
  dplyr::select(-"Ensembl")
```

Let's preview `filtered_mapped_df` to check that we combined the collapsed duplicate Entrez IDs data with the rest of the mapped data.

```{r}
head(filtered_mapped_df)
```

Great, looks like we were able to successfully collapse the duplicate Entrez IDs by keeping the maximum expression value per sample, and combine this collapsed data with the rest of the mapped data!
