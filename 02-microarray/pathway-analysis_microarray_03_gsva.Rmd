...
max_dup_df
```
Looks like we were able to successfully get rid of the duplicate gene identifiers and keep the observations with the maximum expression value!
...
filtered_mapped_df <- mapped_df %>%
  dplyr::filter(
    ...
    !entrez_id %in% dup_entrez_ids |
      # Now let's get the data associated with the duplicated gene identifiers that we
      # collapsed in `max_dup_df`
      entrez_id %in% max_dup_df$entrez_id &
        Ensembl %in% max_dup_df$Ensembl
  )
```

Let's check to see that we removed the duplicate Entrez IDs from the rest of the mapped data.

```{r}
filtered_mapped_df %>%
  # Let's sort by the Entrez IDs we know were duplicated
  dplyr::arrange(dplyr::desc(entrez_id %in% dup_entrez_ids)) %>%
  # Now lt's display the first six observations of our new object
  head()
```

Great, looks like we were able to successfully get rid of the duplicate gene identifiers while joining the rest of the mapped data!
...
filtered_mapped_matrix <- filtered_mapped_df %>%
  # First let's remove the column we no longer need
