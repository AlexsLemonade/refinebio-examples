...
Now let's make a violin plot to look at the differences between the initial, recurrent, re-recurrent, and non-tumoral glioblastoma samples as noted in the `refinebio_disease` column, using our GSVA scores.
...
Now let's make a violin plot so we can look at the differences between the `refinebio_disease` groups in our dataset using our GSVA scores.

```{r}
# Violin plot comparing GSVA scores of the `refinebio_disease` groups in our dataset
violin_plot <- annotated_results_df %>%
  ggplot2::ggplot(ggplot2::aes(
    x = refinebio_disease, # Replace with a variable relevant to your data
    y = gsva_score
  )) +
  ...
  # Add plot labels
  ggplot2::labs(
    title = "GSVA Scores Across Glioblastoma Disease Groups",
    x = "Glioblastoma disease group",
    y = "GSVA score"
  ) +
  ggplot2::theme_bw()
...
Now we can interpret our violin plot showing the distribution of GSVA scores across `refinebio_disease` groups.
Looks like the `non-tumoral` samples are associated with the lowest GSVA scores, while the `recurrent gbm` and `gbm` samples are associated with some of the higher GSVA scores.
