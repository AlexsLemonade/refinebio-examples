...
To run this example yourself, [download the `.Rmd` for this analysis by clicking this link](https://alexslemonade.github.io/refinebio-examples/02-microarray/differential-expression_microarray_01.Rmd).
...
metadata <- metadata %>%
  dplyr::mutate(subgroup = as.factor(gsub("Group 3|Group 4", "NonSHH", subgroup)))
```
...
Now we will create a model matrix based on our newly renamed `subtype` vector. 

```{r}
# Create the design matrix
des_mat <- model.matrix(~subgroup)
```

The design matrix doesn't have super helpful column names, so we will rename them. 

```{r}
# Make the column name more informative
colnames(des_mat)[2] <- paste0(levels(subgroup), collapse = "vs")
```
...
Let's try extracting the data for `ENSG00000012223` and set up its own data frame for plotting purposes. 

```{r}
top_gene_df <- df %>%
  # Extract this gene from `df`
  dplyr::filter(rownames(.) == "ENSG00000012223") %>%
...
Now let's plot the data for `ENSG00000012223` using our `top_gene_df`.

```{r}
ggplot(top_gene_df, aes(x = subgroup, y = ENSG00000012223, color = subgroup)) +
...
ggsave(
  plot = volcano_plot,
  file.path(plots_dir, "GSE37418_volcano_plot.png")
