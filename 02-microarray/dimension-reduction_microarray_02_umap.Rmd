...
In this analysis, we will be using the R package [`umap`](https://www.rdocumentation.org/packages/umapr/versions/0.0.0.9001/topics/umap) [@Konopka2020] for the production of UMAP dimension reduction values and the R package [`ggplot2`](http://r-statistics.co/Complete-Ggplot2-Tutorial-Part1-With-R-Code.html) [@Prabhakaran2016] for plotting the UMAP values.
...
```{r}
# Perform Uniform Manifold Approximation (UMAP)
umap_data <- umap::umap(t(df)) # We have to transpose our data frame to meet the `umap::umap()` function requirements
...
Now, let's label the data points based on their genotype subgroup since this is central to the subgroup specific based hypothesis in the [original paper](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3683624/#SD1) [@Northcott2012].
...
- [Original paper on UMAP](https://arxiv.org/abs/1802.03426) by McInnes, Healy & Melville
[@McInnes2018]
