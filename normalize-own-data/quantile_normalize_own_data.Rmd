...
TODO: This is wrong because the organism filter is broken! https://github.com/AlexsLemonade/refinebio/issues/1224
Pretty sure I am using the mouse target with this method but wanted to get the plotting, etc. prepped!

There are, at the time of this example, 12 quantile normalization targets that
are available.
...
```{r}
colnames(qn_target_info)
```

Let's look at when these were created with some help from the `lubridate` 
package.

```{r}
lubridate::as_datetime(qn_target_info$created_at)
```

We can find the index of the _latest_ QN target with `which.max`.

```{r}
latest_index <- which.max(lubridate::as_datetime(qn_target_info$created_at))
```

For posterity, we're using QN target created at 
**`r lubridate::as_datetime(qn_target_info$created_at)[latest_index]`**
here.
...
```{r}
latest_qn_url <- qn_target_info$s3_url[latest_index]
```
...
```{r}
qn_target <- as.vector(as.matrix(readr::read_tsv(latest_qn_url,
                                                 col_names = FALSE)))
```
