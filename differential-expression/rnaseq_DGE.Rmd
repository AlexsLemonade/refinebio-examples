...
We'll use some tidyverse functionality to extract this information into it's
...
Let's make sure samples are in the same order.
...
```{r}
# put samples in ascending orderaccording to their accession code in the 
# metadata
metadata.df <- metadata.df %>%
...
```{r}
# construct a DGEList object from the refine.bio output
y <- DGEList(as.matrix(expression.df))

# filtering
keep <- filterByExpr(y)
y <- y[keep, ]

# edgeR calculation of normalization factors to scale raw library sizes
y <- calcNormFactors(y)

# design matrix that captures the mutated vs. wild type comparison
design <- model.matrix(~ metadata.df$mutation_status)

# this is the step that transforms data to log2CPM, estimates the mean-variance
# relationship and computes observation-level weights
v <- voom(y, design)  # ready for lmFit
```
