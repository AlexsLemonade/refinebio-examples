...
# Select out what samples are actually hek 293:
hek293 <- grep("hek293|hek-293|hek 293", metadata$refinebio_cell_line, 
               ignore.case = TRUE)
...
```{r}
ggplot(pca.df, aes(x = PC1, y = PC2, color =  metadata$series_id)) +
  geom_point() + 
...
```{r ComBat correction}
corrected.df <- sva::ComBat(as.matrix(df), metadata$refinebio_platform)
```
...
```{r}
ggplot(corrected.pca.df, aes(x = PC1, y = PC2, color = metadata$series_id)) +
  geom_point() + 
