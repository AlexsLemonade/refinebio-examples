...
Do some filtering so we know we are only really looking at HEK293 sets
...
metadata <- metadata[hek293, ] %>% 
  readr::write_tsv("filtered_metadata_HOMO_SAPIENS.tsv")

# Only keep these in the data and metadata
df <- df[, hek293] %>% 
  readr::write_tsv("filtered_HOMO_SAPIENS.tsv")
...
## 5) Save batch corrected data to tsv file
```{r}
corrected.df %>% as.data.frame() %>% 
  tibble::rownames_to_column("gene") %>%
  readr::write_tsv("HEK293_batch_corrected_exprs.tsv")
```
