...
if (!dir.exists(file.path(data_dir,
  ...
  compressed_file <- file.path(data_dir,
                               "GSE37382.zip" # Replace with the name of zipped your file
                               )
...
```{r}
northcott_expression_file <- file.path(data_dir, 
                                       "GSE37382", # Replace with name of your unzipped folder
                                       "GSE37382.tsv" # Replace with name of your expression file
                                       )
...
```{r}
northcott_metadata_file <- file.path(data_dir,
                                     "GSE37382", # Replace with name of your unzipped folder
                                     "metadata_GSE37382.tsv" # Replace with name of your metadata file
                                     )
...
northcott_metadata_df <- northcott_metadata_df %>%
  ...
  dplyr::select(refinebio_accession_code, refinebio_title, refinebio_age, 
...
```{r}
robinson_expression_file <- file.path(data_dir,
                                      "GSE37418", # Replace with name of unzipped folder containing the second expression file
                                      "GSE37418.tsv" # Replace with name of your second expression file
                                      )
...
```{r}
robinson_metadata_file <- file.path(data_dir,
                                    "GSE37418", # Replace with name of unzipped folder containing the second expression file data
                                    "metadata_GSE37418.tsv" # Replace with name of the metadata file relevant to your second expression file
                                    )
...
robinson_metadata_df <- robinson_metadata_df %>%
  # retain only pertinent columns
  dplyr::select(refinebio_accession_code, refinebio_title, age, `m stage`,
                subgroup) %>%
...
```{r}
northcott_results <- qusage(eset = northcott_mat,
...
```{r}
northcott_results_file <- file.path(results_dir,
...
```{r}
robinson_results <- qusage(eset = robinson_mat,
...
```{r}
robinson_results_file <- file.path(results_dir, 
...
legend("topleft", legend = c("Northcott", "Robinson", "Meta-analysis"),
...
```{r}
plotCIsGenes(northcott_results, path.index = 107, addGrid = FALSE,
             cex.xaxis = 1.25)
```
...
```{r}
plotCIsGenes(robinson_results, path.index = 107, addGrid = FALSE,
             cex.xaxis = 1.25)
```
...
```{r}
AnnotationDbi::select(org.Hs.eg.db, keys = c("56907", "56776"), 
