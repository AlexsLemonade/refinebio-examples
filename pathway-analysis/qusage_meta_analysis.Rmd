...
section of the module README](https://github.com/AlexsLemonade/refinebio-examples/tree/master/pathway-analysis#gene-sets).
...
if (!dir.exists(file.path("data", "GSE37382"))) {
  compressed_file <- file.path("data", "GSE37382.zip")
...
```{r}
northcott_expression_file <- file.path("data", "GSE37382", "GSE37382.tsv")
...
```{r}
northcott_metadata_file <- file.path("data", "GSE37382", 
                                     "metadata_GSE37382.tsv")
...
```{r}
robinson_expression_file <- file.path("data", "GSE37418", "GSE37418.tsv")
...
```{r}
robinson_metadata_file <- file.path("data", "GSE37418", 
...
robinson_metadata_df <- robinson_metadata_df %>%
  ...
  dplyr::filter(!(subgroup %in% c("WNT", "SHH OUTLIER"))) %>%
...
```{r}
northcott_results_file <- file.path("results", 
                                    "Northcott_SHH-Other_QSarray.RDS")
...
```{r}
robinson_results_file <- file.path("results", 
                                   "Robinson_SHH-Other_QSarray.RDS")
...
#### Plot individual gene mean and 95% confidence intervals
...
```{r}
plotCIsGenes(northcott_results, path.index = 107)
```
...
```{r}
plotCIsGenes(robinson_results, path.index = 107)
```
