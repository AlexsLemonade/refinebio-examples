...
# other notebook
symbol_file <- file.path("data", "GSE75574", "GSE75574_symbols.tsv")
...
comp_coag_df <- data.frame(
  ...
  ssgsea_score = ssgsea_results["KEGG_COMPLEMENT_AND_COAGULATION_CASCADES", ]
)
```
...
```{r}
# saving to file
ggplot2::ggsave(file.path("plots", "GSE75574_ssgsea_comp_coag_facet.pdf"))
```
...
#### Aside: Gene set _size_ influences ssGSEA score values
...
random_long_df %>%
  ggplot2::ggplot(ggplot2::aes(y = ssgsea_score, x = gene_set_size)) +
  ggplot2::geom_violin() +
  ggplot2::coord_flip() +
