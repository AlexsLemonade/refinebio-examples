---
title: "Pathway analysis with QuSAGE"
output: 
  html_notebook:
    toc: TRUE
    toc_float: TRUE
...
In this notebook, we'll demonstrate how to perform pathway analysis using
...
We will mirror the [`qusage` package vignette](https://bioconductor.org/packages/release/bioc/vignettes/qusage/inst/doc/qusage.pdf) (Bolen C.).
...
This allows us to cover formatting refine.bio datasets for use with 
`qusage`.
We also then have the ability to compare the results using refine.bio to the 
...
**This notebook assumes that you have downloaded the MSigDB v6.2 KEGG gene sets
that use Entrez gene IDs and placed them at the following relative path:**
...
We need to make sure that the KEGG gene sets file exists where we expect it.
...
# it exists where we expect it
if (!file.exists(kegg_file)) {
  stop(paste("KEGG GMT file is not found at", kegg_file))
}
```
...
expression_file <- file.path("data", "GSE30550", "GSE30550.tsv")
exprs_df <- readr::read_tsv(expression_file, progress = FALSE)
# readr will fill in the first column name as 'X1' which is not very informative
...
readr::write_tsv(exprs_df,
                 path = file.path("data", "GSE30550", "GSE30550_entrez.tsv"))
```

### Read in metadata and filter

```{r}
metadata_file <- file.path("data", "GSE30550", "metadata_GSE30550.tsv")
metadata_df <- readr::read_tsv(metadata_file) %>%
  # drop columns that are all NA
  dplyr::select(-which(apply(is.na(.), 2, all)))
```

In the `qusage` vignette, the authors compared Hour 0 samples and sample 77 
hours after exposure to influenza.
...
readr::write_tsv(timepoint_table,
                 file.path("results", "qusage_t1-t0_kegg_results.tsv"))
```
...
readr::write_rds(timepoint_qusage_results,
                 file.path("results", "qusage_t1-t0_kegg_QSarray.RDS"))
```
...
```{r}
plotCIs(timepoint_qusage_results)
```

It's a bit difficult to see in this notebook, but we'll save the plot as a PDF.

```{r}
pdf(file.path("plots", "qusage_t1-t0_plotCIs.pdf"), width = 14, height = 8)
...
```{r}
pathway_index <- 
  which(names(kegg_genesets) == "KEGG_T_CELL_RECEPTOR_SIGNALING_PATHWAY")
```
...
```{r}
twoway_labels <- paste(filtered_metadata_df$clinic_pheno, 
...
```{r}
asx_vs_sx <- qusage(eset = exprs_mat, 
                    labels = twoway_labels, 
...
plotDensityCurves(asx_vs_sx, path.index = 125, col = "#000000", add = TRUE, 
                  lwd = 3)
legend("topleft", 
...
A few genes appeare to be driving the elevation of the cytosolic DNA sensing
