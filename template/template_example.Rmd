...
# Declare the file path to the gene expression matrix file, stored as `data_dir`
data_file <- file.path(data_dir, {{DATA_ACCESSION FILENAME}})

# Declare the file path to the metadata file, stored as `data_dir`
metadata_file <- file.path(data_dir, {{DATA_ACCESSION FILENAME}})
```
...
```{r}
# Check if the gene expression matrix file is where the file path we specified says
file.exists(data_file)

# Check if the metadata file is where the file path we specified says`
...
We specified our file paths in [this previous step](#check-out-our-file-structure) and have them stored as objects named `metadata_file` and `data_file`. 
